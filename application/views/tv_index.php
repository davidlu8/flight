...
<div class="wrapper">
    ...
    <?php echo '<pre>';print_r($item);print_r($userinfo) ?>
    <?php if ($item['USERATTR_TV'] == 1): ?><span class="text-danger">神秘人</span><?else:?><a href="userID:<?php echo $ownerInfo['USERINFO_USER_ID'] ?>" class="text-danger"><?php echo $ownerInfo['USERINFO_NICKNAME'] ?></a><?php endif ?>送给了<a href="userID:<?php echo $userInfo['USERINFO_USER_ID'] ?>" class="text-danger"><?php echo $userInfo['USERINFO_NICKNAME'] ?></a><?php echo $item['GIFTHISTORY_GIFT_AMOUNT'] ?>个<span class="text-success">[<?php echo $gift['GIFT_NAME'] ?> <img src="<?php echo $gift['GIFT_IMAGE'] ?>" width="28" />]</span>,
